...
# Clean the data by removing rows with missing Age-adjusted Death Rate or Year values
df = df.dropna(subset=["Age-adjusted Death Rate", "Year", "Cause Name", "State"])

# Define the sidebar
st.sidebar.title("Select cause of death:")
cause_of_death = st.sidebar.selectbox("Cause of death:", df["Cause Name"].dropna().unique(), key="cause_of_death_selector")
...
filtered_df = filtered_df[filtered_df["Age-adjusted Death Rate"] > 0]  # Remove invalid values
...
# Define dictionaries for cause descriptions
cause_descriptions = {
    ...
    "Suicide": "Suicide is the act of deliberately killing oneself. Risk factors for suicide include mental disorder, especially depression, and neurological disorders, cancer and HIV infection. Suicide is a serious public health problem that can have lasting harmful effects on individuals, families, and communities. There are many factors that contribute to suicide. The goal of suicide prevention is to reduce factors that increase risk and increase factors that promote resilience. For more information visit [cdc.gov](https://www.cdc.gov/suicide/) .",
...
if active_tab == "Overview":
    st.markdown('''# Age-Adjusted Death Rates in the United States, 1999-2017''')
    ...
    st.write("# Age-Adjusted Death Rate over Time in Each State")
    states = df['State'].unique().tolist()
    selected_state = st.selectbox('Select a state:', states)
    state_df = df.loc[(df['State'] == selected_state) & (df['Cause Name'] == cause_of_death)]
    avg_rate = state_df['Age-adjusted Death Rate'].mean()
    bar_data = state_df.groupby(["Year"])[["Age-adjusted Death Rate"]].mean().reset_index()
    chart = alt.Chart(bar_data).mark_line(color="green").encode(
        x=alt.X("Year", axis=alt.Axis(format="")),
        y=alt.Y("Age-adjusted Death Rate", scale=alt.Scale(zero=False))        
    ).properties(
        title=f'Age-Adjusted Death Rates for {cause_of_death} in {selected_state}.'
    )
    st.altair_chart(chart, use_container_width=True)
    ...
    if active_tab == "Descriptions of cause of death":
        ...
        st.markdown(f"<details><summary>Click to read the description of {cause_of_death}</summary><p>{cause_descriptions.get(cause_of_death, 'No description available.')}</p></details>", unsafe_allow_html=True)
        st.write(f"Age-adjusted death rate for {cause_of_death} in the United States : {age_adjusted_rate:.2f}")
